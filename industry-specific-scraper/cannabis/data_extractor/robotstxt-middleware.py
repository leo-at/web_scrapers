...
class CustomRobotsTxtMiddleware(object):
    DOWNLOAD_PRIORITY = 1000

    def __init__(self, crawler):
        if not crawler.settings.getbool('ROBOTSTXT_OBEY_CUSTON'):
            raise NotConfigured
    ...
    def process_request_2(self, rp, request, spider):
        if rp is not None and not rp.can_fetch(
            ...
            logger.debug("Forbidden by robots.txt (Custom): %(request)s",
    ...
    def process_exception(self, request, exception, spider):
        ...
        #print("process_exception: {}".format(ex_class)) 
        if ex_class == 'scrapy.exceptions.IgnoreRequest':
            url = urlparse_cached(request)
            robotsurl = "%s://%s/robots.txt" % (url.scheme, url.netloc)
            spider.middleware_exception(request, self.__class__.__name__, robotsurl)
